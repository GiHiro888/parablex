...
| [parablex-writing](parablex-writing/SKILL.md) | 文書・仕様・提案の作成／推敲 | 「文書作成」「推敲」「writing」 |
| [parablex-plan](parablex-plan/SKILL.md) | 計画・設計のストレステスト | 「計画レビュー」「設計レビュー」「plan」 |
...
parablex-kit/
  ...
  VERIFICATION.md              # 「機能した」証明の手順（ルーブリック＋プロトコル）
  ...
  verification/
    baseline-claude.md             # 基準実行（Claude）— 突合用の参照解
```

## 検証（機能した証明）
「動いた」ではなく**証拠で示す**。[VERIFICATION.md](VERIFICATION.md) が4段階（起動→形式準拠→内容妥当→移植性）・9原則スコアカード・真陽性サンプリング法を定義。突合用の参照解：[`origin/`](origin/)（Fable 5・由来）と [`verification/baseline-claude.md`](verification/baseline-claude.md)（Claude）。
